...
def update_messages(payload, role, content):
...
def calculate(user_query):
    ...
    # Checking the Acceptence condition
    data_knc["Customer Label"] = (data_knc['DSO'] > 45) | (data_knc['Delay %'] > 50) | (
                data_knc['Ontimepay %'] < 75) | (data_knc['OverdueInvoice%'] > 5)
    data_knc["Customer Label"] = data_knc["Customer Label"].replace(True, 'RED')
                ...
                while True:
                    ...
                    try:
                        ...
                        if matches:
                            ...
                            payload = update_messages(payload, 'user', db_details.user_query2)
                            return {'solution': output, 'final_answer': output}
                    except:
                        code = "#"
                # Code execution
                exec(code0 + code)
