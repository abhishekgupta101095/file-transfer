...
class Classifier:
    ...
    def detect_language(self, data):
        # print(f"Inside detect_language")
        lang_response = ""
        ...
        for filepath in data:
            ...
            try:
                ...
                pdf_file = PdfReader(BytesIO(contents))

                for page in pdf_file.pages:  # Access the pages attribute
                    final_response.append(page.extract_text())
                # print(f"final response is {final_response}")
            except Exception as error:
                print(error)

            # lang_text = final_response[0]
            lang_text = extracted_text[:4000]
            
            try:
                print(f"checking document language")
                print(f"language text to validate:: {lang_text}")
                response = COMPREHEND_CLIENT.detect_dominant_language(Text=lang_text)
                lang_response = response["Languages"]
                lang_code = lang_response[0]["LanguageCode"]
            except Exception as error:
                print(error)    

        # print(f"extracted test in detect_language :: {final_response}")  
        return lang_code
        ...
        if data:
            ...
            for filepath in data:
                ...
                if is_document_translated == "Y":
                    ...
                    try:
                        temp_filename = Path(Path(file_name).parent).joinpath("temp.pdf")
                ...
                try:
                    for i in range(3):
                        try:
                            ...
                            print(f"endpoint arn is - {endpoint_new}")
                            response = COMPREHEND_CLIENT.classify_document(
                            EndpointArn=endpoint_new,
                            Bytes=img_data_new,
...
#                         result["Classes"] = response["Classes"]
                        if trimmed_output == "Y":
                            ...
                            # result["phidata"] = {'Entities': [{'BeginOffset': 98,
