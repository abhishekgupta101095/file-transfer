...
def move_s3_objects(key: str, s3_prefix: str) -> None:
    ...
    destination_key = newKey.replace(S3_DICTIONARY_PREFIX, s3DictionaryProcessedPath, 1)
...
with DAG(dag_id="loadMetadata", default_args=default_args, schedule_interval=None) as dag:
    
    '''metadataJobSensor = ExternalTaskSensor(
        task_id='metadataJobSensor_task',
        external_dag_id='downloadFilesAndDQCheck',
    ...
    listMetadataFilesLocal = PythonOperator(
        task_id='list_files_task',
        python_callable=list_files_and_locations,
        op_args=[METADATA_LOCAL_PATH],
        provide_context=True
    )
          
    metadata_processing = PythonOperator.partial(
        task_id='metadata_processing',
        python_callable=process_s3
        ).expand(
          op_kwargs=listMetadataFilesLocal.output.map(
            lambda x: {
                "filePath":f"{x}"
            }
        )
    ...
    triggerLoad3pIndividual = TriggerDagRunOperator(
        task_id="triggerLoad3pIndividual",
        trigger_dag_id="load3pIndividual"
...
start >> listMetadataFilesLocal >> metadata_processing >> listMetadataFilesS3 >> move_files_task
move_files_task >> delete_s3_objects >> triggerLoad3pIndividual >> end
