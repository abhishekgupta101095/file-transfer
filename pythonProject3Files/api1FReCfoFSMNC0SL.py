...
def get_df():
    ...
    df_r = data
    df_r['DateMonth'] = df_r['DateMonth'].astype(str)
    df_r['YEAR'] = df_r['DateMonth'].str.slice(0, 4)
    df_r['MONTH'] = df_r['DateMonth'].str.slice(4, 6)
    # df['QUARTER'] = np.where(df['MONTH'].isin(q1m), df['Budget'] * 0.78125, df['Budget'])
    df_r['DT_CONV'] = pd.to_datetime(df_r['DateMonth'], format='%Y%m', errors='coerce').dropna()
    df_r['QUARTER'] = df_r['DT_CONV'].dt.quarter
    return df_r

class DBcls(BaseModel):

    df_c = get_df()
    years: Optional[list] = list(df_c['YEAR'].drop_duplicates())
    col_name: Optional[list] = list(df_c.columns)
        # ['G_L_AccountDescription', 'Company_CodeDescription', 'DateMonth', 'CategoryVersion', 'Cost_CenterDescription', 'Profit_CenterDescription', 'ProductDescription', 'CustomerDescription', 'Distribution_ChannelDescription', 'Value', 'QUARTER']
    category: Optional[str] = 'Actual'
    months: Optional[list] = list(df_c['MONTH'].drop_duplicates())
        # ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']
    quarters: Optional[list] = list(df_c['QUARTER'].drop_duplicates())
        # [1, 2, 3, 4]
    product_name: Optional[list] = list(df_c['ProductDescription'].drop_duplicates())
    # ['A', 'B', 'C','D','E','F', 'Unassigned']
    company_code: Optional[list] = list(df_c['Company_CodeDescription'].drop_duplicates())
        # ['EU','US']
    profit_center_name: Optional[list] = list(df_c['Profit_CenterDescription'].drop_duplicates())
...
async def answer_question(db_details: DBcls, ans: bool = True):
    ...
    me_data_list = [
        ...
        },
        {
            ...
            "data_type": "object",
            "sample_data": ['Opening Cash', 'Equity Shares Outstanding', 'Interest Expense ', 'Hub Expenses ',
                            'Market Capitalization', 'Non Op Expenses ', 'Market Price per share', 'Operations',
                            'Payroll Tax & Fringe ', 'Standard Cost ', 'Investments', 'MB-4001--BOA01-USD01 ',
                            'Depreciation Expense ', 'Financing', 'Customer Rcvbls ', 'Inventory ',
                            'FA Build & Lease Imp ', 'Dividend per Share', 'AD Build & Lease Imp ', 'Sponsorships ',
                            'LT Investments ', 'AA Patents ', 'Vendor Payables - Do ', 'Cur Inc Tax-Natl ',
                            'AL Payroll ', 'Inc Tax Pay-ST/Prov ', 'Other Current Liab ', 'LT of LT Conv Debt ',
                            'Ord Shares / Comm St ', 'CY Ret Earnings '],
        ...
        },
        {
            ...
            "data_type": "object",
            "sample_data": ['Unassigned', 'Business market Canada', 'Business market Brazil',
                            'Business market Argentina', 'Business market Australia'],
    ...
    finlist = []
    for i in range(len(me_data_list)):
        if me_data_list[i]['column_name'] in db_details.col_name:
            finlist.append(me_data_list[i])
    system_message = system_message + str(finlist)
    system_message2 = system_message2 + str(finlist)
