...
def Calc(dfc, cc='', ncc=''):
    ...
    operating_Exp = dfc[dfc['G_L_AccountDescription'].str.contains('Depreciataion|Expense|Sponsorships|Fringe') & ~dfc[
        'G_L_AccountDescription'].str.contains('Prov|Non')]['Value'].sum()
    ...
    net_assets = \
    (dfc[dfc['G_L_AccountDescription'].str.contains('Lease|LT Investments|Patents|MB-4001|Rcvbls|Inventory')])[
        'Value'].sum()
    ...
    net_ebit = net_revenue - \
               (dfc[dfc['G_L_AccountDescription'].str.contains('Fringe|Cost|Sponsorship|Depreciation|Hub')])[
                   'Value'].sum()
    ...
    while 1:
        ...
        try:
            if counter < 5:
                ...
                exec(code0 + code)
                try:
                    final_answer = pd.read_csv('output.csv',index_col = 0)
                    ...
                    for i in final_answer.columns:
                        if isinstance((final_answer[i][0]), np.float64) :
                                    if (abs(final_answer[i].min())) > 100000000:
                                        final_answer[i] = final_answer[i] / 1000000000
                                        final_answer.rename(columns={i: str(i) + '(in billions $)'}, inplace=True)

                                    # elif '%|$' not in i:
                                    #     final_answer.rename(columns={i: str(i) + '($)'}, inplace=True)
